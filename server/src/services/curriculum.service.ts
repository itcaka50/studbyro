...
export const updateCurriculumCourse = async (
    ...
    const updated = await CurriculumCourse.query()
        .patch({ credits })
        .where({ curriculumId, courseId });

    if (!updated) {
        throw new Error('Курсът не е част от този учебен план!');
    }

    return await CurriculumCourse.query().findOne({ curriculumId, courseId });
};
...
export const removeCourseFromCurriculum = async (
    ...
    const deleted = await CurriculumCourse.query()
        .delete()
        .where({ curriculumId, courseId });
