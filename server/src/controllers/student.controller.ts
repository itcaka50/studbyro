...
export const getMyCourses = async (
    ...
    try {
        const facultyNumber = res.locals.user.student.facultyNumber;
        const courses =
            await studentService.getStudentCoursesAndGrades(facultyNumber);
        res.status(200).json({
            success: true,
            count: courses.length,
            data: courses,
...
export const getMyAvailableCourses = async (
    req: Request,
    res: Response,
    next: NextFunction,
) => {
    try {
        const facultyNumber = res.locals.user.student.facultyNumber;
        const courses =
            await studentService.getAvailableCoursesForStudent(facultyNumber);
        res.status(200).json({
            success: true,
            count: courses.length,
            data: courses,
...
export const enrollMyCourse = async (
    req: Request,
    res: Response,
    next: NextFunction,
) => {
...
export const getMySchedule = async (
    ...
    try {
        const facultyNumber = res.locals.user.student.facultyNumber;
        const schedule = await studentService.getStudentSchedule(facultyNumber);
        res.status(200).json({
            success: true,
...
export const getMyProgram = async (
    ...
    try {
        const facultyNumber = res.locals.user.student.facultyNumber;
        const program = await studentService.getStudentProgram(facultyNumber);
        res.status(200).json({
            success: true,
